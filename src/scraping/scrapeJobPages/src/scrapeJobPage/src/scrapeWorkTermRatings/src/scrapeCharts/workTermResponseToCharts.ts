...
import {
  IWorkTermRatingResponse,
  IWorkTermRatingTableSection,
} from "src/scraping/scrapeJobPages/src/scrapeJobPage/src/scrapeWorkTermRatings/src/scrapeCharts/src/types/WorkTermRatingResponse";
import { removeHtmlTags } from "src/utils/strings/removeHtmlTags";
import { trimWithRegex } from "src/utils/strings/trimWithRegex";

export const workTermReponseToCharts = (
  org: string,
  response: IWorkTermRatingResponse,
): IScrapedJob["charts"] => {
  ...
  const hiringHistory = sections.find(
    (x) => x.title.toLowerCase() === "hiring history" && x.type === "table",
  ) as IWorkTermRatingTableSection | undefined;
  if (!hiringHistory) {
    return undefined;
  }

  const totalHires =
    hiringHistory.rows[1]?.reduce((acc, curr) => {
